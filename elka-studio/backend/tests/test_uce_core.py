...
class DummyAIAdapter(BaseAIAdapter):
    ...
    def __init__(
        self, response: object, json_responses: Iterable[str] | None = None
    ) -> None:
    ...
    def generate_markdown(
        self, instruction: str, context: str | None = None
...
def universe_repo(tmp_path: Path) -> Path:
    ...
    (tmp_path / "Legendy" / "myth.md").write_text(
        "# Forgotten Myth\n\n- Core truth\n", encoding="utf-8"
    )
    (tmp_path / "timeline.txt").write_text(
        "1200 Founding of the order\n", encoding="utf-8"
    )
    return tmp_path
...
def test_extract_fact_graph_retry_on_invalid_json() -> None:
    responses = [
        "not-json",
        json.dumps({"entities": [], "events": [{"title": "Duel"}]}),
    ]
...
def test_plan_changes_creates_new_files(universe_repo: Path) -> None:
    incoming = FactGraph(
        entities=[FactEntity(id="village", type="place", summary="Quiet village")]
    )
    ...
    assert "# village" in next(
        file.new for file in changeset.files if file.path == "Objekty/village.md"
    )
...
def test_plan_changes_adds_timeline_events(universe_repo: Path) -> None:
    incoming = FactGraph(
        events=[
            FactEvent(
                id="battle", title="Battle of Spring", date="1201", participants=[]
            )
        ],
    )
    changeset = plan_changes(FactGraph(), incoming, universe_repo)
    timeline_change = next(
        file for file in changeset.files if file.path.startswith("timeline")
    )
...
def test_plan_changes_timeline_duplicate_detection(universe_repo: Path) -> None:
    ...
    timeline_file = next(
        file for file in initial.files if file.path.startswith("timeline")
    )
...
def test_plan_changes_uses_writer_for_body(universe_repo: Path) -> None:
    ...
    incoming = FactGraph(
        entities=[FactEntity(id="artifact", type="artifact", summary="Ancient key")]
    )

    changeset = plan_changes(FactGraph(), incoming, universe_repo, writer)

    new_file = next(
        file for file in changeset.files if file.path == "Objekty/artifact.md"
    )
...
def test_validate_universe_missing_entity() -> None:
    incoming = FactGraph(
        events=[FactEvent(id="raid", title="Raid", participants=["ghost"])]
    )
    issues = validate_universe(FactGraph(), incoming)
    assert any(
        issue.code == "missing_entity" and "ghost" in issue.refs for issue in issues
    )
...
def test_validate_universe_legend_breach() -> None:
    ...
    incoming = FactGraph(
        events=[FactEvent(id="fall", title="Hero falls", participants=[])]
    )
    ai = LegendAIAdapter(
        [{"message": "Contradiction detected", "refs": ["fall"], "level": "error"}]
    )
...
def test_validate_universe_loads_template_truths() -> None:
    class RecordingAdapter(LegendAIAdapter):
        def __init__(self) -> None:
            super().__init__(
                [{"message": "conflict", "refs": ["fall"], "level": "error"}]
            )
    ...
    issues = validate_universe(
        FactGraph(), FactGraph(events=[FactEvent(id="fall", title="Hero falls")]), ai
    )
...
def test_git_adapter_branch_and_commit(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "Objekty" / "hero.md").read_text(
        encoding="utf-8"
    ) == "# hero\nChampion\n"
