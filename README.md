...
- **Universe browser** – Explore repository directories with collapsible tree navigation, inline filtering, and a polished read-only preview.
- **Text-to-speech previews** – Listen to Universe Browser file contents using dedicated play/pause/stop controls powered by the Web Speech API.
...
- Missing entities discovered during story imports now register as informational notes, and legend breaches only block the run when paired with critical conflicts (for example `entity_type_conflict`). The Celery task logs non-blocking inconsistencies as warnings so empty universes can accept new canon while still surfacing actionable audit logs.
...
- Import existing stories in bulk with `POST /api/projects/{project_id}/import-stories`. Send a `multipart/form-data` payload with one or more `files` fields (`.txt` or `.md`). The backend stores every upload under `Stories/Imported_<timestamp>/`, registers a `uce_process_story` task that tracks the sequential Universe Consistency Engine run, and returns both the numeric `task_id` and the Celery identifier so dashboards can follow progress without rescanning the filesystem. Database transaction handling has been hardened so scheduling these tasks no longer triggers `A transaction is already begun on this Session.` errors after earlier queries in the same request cycle.
