...
- Import existing stories in bulk with `POST /api/projects/{project_id}/import-stories`. Send a `multipart/form-data` payload with one or more `files` fields (`.txt` or `.md`). The backend stores every upload under `Stories/Imported_<timestamp>/`, registers a `uce_process_story` task that tracks the sequential Universe Consistency Engine run, and returns both the numeric `task_id` and the Celery identifier so dashboards can follow progress without rescanning the filesystem.
